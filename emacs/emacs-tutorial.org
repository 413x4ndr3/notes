...
- =C-k= will kill a Newline character only if the cursor is on it: =C-k= twice to kill a whole line
  - _However =C-u <n> C-k>= will kill =n= lines with their Newline character=
...
- _A numeric ARG to =undo= serves as a _repeat count_
...
- _In Elisp, =switch-to-buffer= is NOT the way to work on another buffer temporarily_ \rightarrow Use =set-buffer= insted
...
- _Switching between buffers do not save any of them on disk__
...
| =C-s= | =isearch-forward=  | Do incremental search forward (With a prefix argument, do an icremental regular expression search instead)  |
| =C-r= | =isearch-backward= | Do incremental search backward (With a prefix argument, do an icremental regular expression search instead) |
...
- _=C-s=/=C-r= search string AFTER/BEFORE point (they both do both actually)_
...
- Therer are both cursor motion command
