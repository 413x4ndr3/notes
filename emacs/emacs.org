...
- =cs:= describe the *character set* and *newline convention* used for the *current buffer*.
  ...
  - if =-=: *no special character set* handling (with the possible exception of end-of-line conventions)
  - if ===: *no conversion* whatsoever \rightarrow usually for files with non-textual data
