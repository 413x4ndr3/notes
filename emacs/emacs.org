...
- The buffer displayed in the selected window is called the *current buffer*: _this is where editing happens_.
...
- When using multiple frames (= when using a graphical display), _selecting a particular frame selects a window in that frame_.
