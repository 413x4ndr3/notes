...
| Key                  | Command                | Description                                                                                |
|----------------------+------------------------+--------------------------------------------------------------------------------------------|
| =DEL (or BACKSPACE)= | =delete-backward-char= | *Delete the character before point*, or the region if it is active                         |
| =Delete=             | =delete-forward-char=  | *Delete the character after point (i.e., under the cursor)*, or the region if it is active |
| =C-d=                | =delete-char=          | *Delete the character after point (i.e., under the cursor)*                                |
|                      |                        |                                                                                            |
| =C-k=                | =delete-char=          | *Kill to the end of line*                                                                  |
| =M-d=                | =kill-word=            | *Kill forward to the end of the next word*                                                 |
| =M-DEL=              | =backward-kill-word=   | *Kill back to the beginning of the previous word*                                          |


- =DEL= \neq =Delete= key
  - (Emacs may not recognize the =DEL= key properly (See Section [DEL Does Not Delete] if this problem occurs)

- The difference between =Delete= and =C-d= is that =C-d= works *regardless of whether the region is active* \rightarrow *The region becomes unactive*

- (=DEL= move point and the characters after it *backwards*)
- (=Delete= does not move point but *move characters after point backwards* as well)
  - (The next/previous line is *merged* with the current one *if a newline character is deleted*) 
  
- (If point was at the BOL, this deletes the preceding newline character, joining this line to the previous one \rightarrow Analogous behavior for =Delete= but at EOL)

- =C-k= will kill a *newline character only if it is under the cursor* \rightarrow Invoke =C-k= twice to kill a whole line (or use the new =C-S-DEL=)
  - However, *=C-u /n/ C-k= will kill /n/ lines* (with their newline character)

- (See Chapter 9 [Killing], and Section [Deletion], for details)


** Undoing Changes

- Emacs records a *list of changes made in the buffer text*, so you can undo/redo recent changes

| Key                     | Command | Description                        |
|-------------------------+---------+------------------------------------|
| =C-/ (or C-x u or C-_)= | =undo=  | Undo one entry of the undo records |


- Most of the time: *1 command = 1 undo entry*
  - Very simple commands *may be grouped together* (E.g., kill commands)
  - However, sometimes an entry may cover just part of a complex command

- *The undo command applies only to changes in the buffer* \rightarrow *Cursor motion commands are not stored*

- When undoing the laste change, *point moves back where it was before the change*

- *=undo= invocation can be repeated*: each repetition undoes another earlier change \rightarrow Until the undo stack is empty (display an error messages when reached)

- (See Section [Undo], for details)
...
| Key       | Command       | Description                                                                                              |
|-----------+---------------+----------------------------------------------------------------------------------------------------------|
| =C-x C-f= | =find-file=   | Edit file /FILENAME/ or switch to a buffer visiting file /FILENAME/, *creating a new empty buffer* if none already exists |
| =C-x C-s= | =save-buffer= | Save current buffer in visited file if modified (+ variations for backup files using /ARG/)              |

- Text that you insert in an Emacs buffer *lasts only as long as the Emacs session* \rightarrow Must be put in a file to *save changes permanently*

- !!! *Emacs obeys the =find-file= command by /visiting/ the file: it creates a buffer, copies the contents of the file into the buffer, and then displays the buffer for editing*
  - *If text is altered, =save-buffer= copies the buffer contents back into the visited file, making it permanent* \rightarrow *Otherwise the altered text exists only inside the Emacs session (and the visited file is unaltered)*
  - (A new empty buffer created for a non-existing visited file on disk \rightarrow *The file is create the file only on the first invocation of =save-buffer=*)

- (=find-file= uses the minibuffer to read the argument that is given to it \rightarrow =RET= terminate the argument)

- (See Chapter 15 [Files], for details)
...
- =(F1= is an alias of =C-h)

- =describe-functions= *describes existing functions* (and not only commands)

- (See Chapter 7 [Help], for details)

** Blank Lines

| Key       | Command              | Description                                                                                 |
|-----------+----------------------+---------------------------------------------------------------------------------------------|
| =C-o=     | =open-line=          | Insert a newline and *leave point before it* (i.e., insert a blank line *after the cursor*) |
| =C-x C-o= | =delete-blank-lines= | Delete all but one of many consecutive blank lines                                          |
| =RET=     | =newline=            | Insert a newline, and move to left margin of the new line if it's blank                     |


- Consider using =C-o= to *space out* your lines of text instead of typing =RET= + =C-p=
  - (Prefer spacing out your line *before* inserting your text in order to make it *easier to see what you are typing*)

- Consider passing an /ARG/ to =C-o= to create /n/ lines instead of typing =C-o= multiple times
